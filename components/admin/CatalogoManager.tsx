...
export default function CatalogoManager({
  ...
  const iniciarEdicionMaestro = (p: ProductoBase & { maxCantidad?: number }) => {
                      ...
                      <td colSpan={4} className="px-4 py-4">
                        <div className="grid gap-3 sm:grid-cols-2">
                          ...
                          <div className="flex flex-col gap-1 sm:col-span-2">
                            <label className="text-xs font-medium text-gray-600">Unidad de venta</label>
                            <div className="flex gap-2">
                              {(["Pz", "Kg"] as const).map((u) => (
                                <button
                                  key={u}
                                  type="button"
                                  onClick={() => setFormEditar((f) => ({ ...f, unidad: u }))}
                                  className="rounded-lg px-4 py-2 text-sm font-semibold transition"
                                  style={formEditar.unidad === u
                                    ? { background: "#15803d", color: "#fff" }
                                    : { background: "#f3f4f6", color: "#374151", border: "1px solid #d1d5db" }}
                                >
                                  {u === "Pz" ? "Pieza (Pz)" : "Kilogramo (Kg)"}
                                </button>
                              ))}
                            </div>
                          </div>
                  ...
                  <div className="flex gap-2">
                    {(["Pz", "Kg"] as const).map((u) => (
                      <button
                        key={u}
                        type="button"
                        onClick={() => setFormNuevo((f) => ({ ...f, unidad: u }))}
                        className="rounded-lg px-4 py-2 text-sm font-semibold transition"
                        style={formNuevo.unidad === u
                          ? { background: "#15803d", color: "#fff" }
                          : { background: "#f3f4f6", color: "#374151", border: "1px solid #d1d5db" }}
                      >
                        {u === "Pz" ? "Pieza (Pz)" : "Kilogramo (Kg)"}
